...
async function main() {
  console.log("Deploying PrivacyPool contract with TlsnVerificationLib...");
  ...
  // Deploy TlsnVerificationLib library
  console.log("Deploying TlsnVerificationLib library...");
  const TlsnVerificationLib = await hre.ethers.getContractFactory("TlsnVerificationLib");
  const tlsnVerificationLib = await TlsnVerificationLib.deploy();
  await tlsnVerificationLib.waitForDeployment();
  console.log(`TlsnVerificationLib deployed to: ${tlsnVerificationLib.target}`);
  ...
  // Deploy PrivacyPool with library linking (TlsnVerificationLib not needed for linking)
  ...
  return {
    ...
    merkleTreeLib: merkleTreeLib.target,
    poseidonT3: poseidonT3.target,
    tlsnVerificationLib: tlsnVerificationLib.target,
...
if (require.main === module) {
  main()
    .then((addresses) => {
      ...
      console.log("TlsnVerificationLib:", addresses.tlsnVerificationLib);
